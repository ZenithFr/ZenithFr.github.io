...
See `references/fallback-providers.md` for configuring model fallback chains (e.g., escalating from deepseek to gemini variants) to handle token exhaustion or rate limits.

## Optimization Guide
See `references/optimization-guide.md` for configuration patterns that reduce token usage: fallback providers, toolset thinning, compression tuning, model selection for instruction adherence, and memory budget management.
...
### Troubleshooting Messaging Gateways
- **Check status**: `hermes gateway status`
- **Verify platform configuration**: `hermes gateway setup`
- **Read logs**: `cat ~/.hermes/logs/gateway.log`
- **Persistence**: Install as a background service (`hermes gateway install`). Use `hermes gateway start`.
- **Known Pitfall**: If `gateway run` is interrupted or times out, the service may be stuck. Ensure the process is not running in the background before starting a new session. Always prefer installing as a service for production or persistent needs.
